...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/11 21:51:16 by pcalime           #+#    #+#             */
/*   Updated: 2016/04/12 00:59:07 by pcalime          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_bns_img(t_data *data, t_fract ft)
{
	...
	while (++x < ft.image_x)
	{
		...
		while (++y < ft.image_y)
		{
			...
			{
				tmp = ft.z_r;
				ft.z_r = ft_abs(ft.z_r * ft.z_r - ft.z_i * ft.z_i - ft.c_r);
				ft.z_i = ft_abs(2 * ft.z_i * tmp + ft.c_i);
			}
...
void			ft_bns(void)
{
	...
	fract = ft_init_bns();
	data->mlx = mlx_init();
	data->win = mlx_new_window(data->mlx, SIZE_WIN, SIZE_WIN, "burningship");
